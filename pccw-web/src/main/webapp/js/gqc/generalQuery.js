var generalQuery = function(){
	...
	var delDataResource = function(ids, grid){
		dataGrid = grid;
		$.post(baseUrl + 'delete.do', {ids:ids}).done(function(data){
	...
	var fillSelectData = function(id){
		$.get(baseUrl + 'findAllDataSource.do').done(function(data){
			...
			dataSourceSelect.select({
				...
				contents: contents
					...
					var url = baseUrl + 'findAllTable.do?id='+ dataSourceId;
					$.get(url).done(function(data){
						...
						if(id){
							dialog.trigger('dataSourceSelectComplete.do')
						}else{
		...
		tableSelect.select({
			title: '选择表'
				...
				var url = baseUrl + 'findAllColumn.do?id='+id+'&tableName='+tableName;
				$.get(url).done(function(data){
					clearSelectedItem();
					fillLeftTable(data.tableMap, data.tableMap)
					dialog.trigger('tableSelectComplete.do')
				});
		});
	};
	/*
	...
	var setData = function(id){
		$.get(baseUrl + 'getById.do?id='+id)
			.done(function(data){
				...
				dialog.on('dataSourceSelectComplete.do',  function(){
				...
				dialog.on('tableSelectComplete.do', $.proxy(fillRightTable, this));
	...
	* 数据验证  成功返回true  失败返回false
	...
	var preview = function(id, dataSourceId){
		$.get(contextPath + "/dataSource/checkDataSourceById.do?id=" + dataSourceId).done(function(data){
			if (data.result == "该数据源不可用") {
				...
				var previewWindow = window.open(contextPath + '/previewTemplate/'+id+'.do', '预览');
