var department = function(){
	...
	var del = function(org, type, $element){
        delete org.title;
		var url = 'terminate-department.do';
		if(type == 'Company'){
			url = 'terminate-company.do';
		}
			...
			save(id, type, organizationType);
				...
				'complete': function(event, data){
                    ...
                    if(type == 'updateCompany' || type == 'updateDepartment'){
                    	 $.get(contextPath + '/organization/getOrg.do?id='+id).done(function(data){
 	                        var org = data.org;
 	                        $element.data(org);
 	                        if(!$element.hasClass('tree-item')){
	...
	var setData = function(id){
		$.get(baseUrl+'getOrg.do?id='+id).done(function(data){
			org = data.org;
	...
	var save = function(id, type, organizationType){
		...
		switch(type){
			case 'addDepartment':
				url =  baseUrl + 'create-department.do?parentId='+id+'&parentType='+organizationType;
				break;
			case 'addCompany':
				url =  baseUrl + 'create-company.do?parentId='+id;
				break;
			case 'updateCompany':
				url =  baseUrl + 'update-company.do';
				break;
			case 'updateDepartment':
				url =  baseUrl + 'update-department.do';
				break;
		}
	...
	var getTree = function(id){
        ...
        $.get(baseUrl + 'orgTree.do').done(function(data){
	...
	var loadEmployeeList = function(employeeListDialog, id){
		...
		departmentEmployeeGrid.grid({
			...
			url:  contextPath + '/employee/pagingquery-by-org.do?organizationId='+id
		});
		...
		grid.confirm({
			...
			callBack: function(){
				$.ajax({
					...
					'type': "Post",
					'url':  contextPath + '/organization/terminate_eoRelations.do?organizationId=' + id,
