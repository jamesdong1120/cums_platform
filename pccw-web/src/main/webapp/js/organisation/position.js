var position = function(){
	...
	var remove = function(objects, grid){
		...
		$.ajax({
		    ...
		    'type': "Post",
		    'url': baseUrl + 'terminate-posts.do',
	...
	var loadJobList = function(id){
		$.get( contextPath + '/job/query-all.do').done(function(data){
			var items = data.data;
	...
	*   保存数据 id存在则为修改 否则为新增
	...
	var save = function(id){
		...
		var url = baseUrl + 'create.do?organizationId='+departmentInput.val();
		if(id){
			url =  baseUrl + 'update.do?organizationId='+departmentInput.val();
		}
