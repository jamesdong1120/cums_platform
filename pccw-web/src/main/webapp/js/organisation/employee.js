var employee = function(){
	...
	var remove = function(objects, grid){
		...
		$.ajax({
		    ...
		    'type': "Post",
		    'url': baseUrl + 'terminate-employees.do',
	...
	var  fillGenders = function(){
		$.get( contextPath + '/employee/genders.do').done(function(data){
			var items = data.data;
	...
	var  fillPosts = function(organizationId){
		$.get( contextPath + '/post/query-post-by-org.do?organizationId='+organizationId).done(function(data){
	...
	*   保存数据 id存在则为修改 否则为新增
	...
	var save = function(id){
		...
		if(id){
			...
			url = baseUrl + 'create.do?postId='+post.getValue();
		}
