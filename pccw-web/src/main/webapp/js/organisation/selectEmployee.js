...
var selectEmployee = function(){
		...
		$.get( contextPath + '/pages/organisation/selectEmployeeTemplate.jsp').done(function(data){
			...
			dialog.find('#save').on('click',function(){
				...
				$.ajax({
				    ...
				    'type': "Post",
				    'url':  contextPath + '/organization/assign-employees-to-organization.do?organizationId='+hostOrgId,
	...
	var loadDepartmentTree = function(hostOrgId){
		departmentTree.tree({
			url:  contextPath + '/organization/orgTree.do',
	...
	var loadEmployeeList = function(id, hostOrgId){
		...
		employeeGrid.grid({
			...
			url:  contextPath + '/employee/pagingquery-by-org.do?organizationId='+id+'&hostOrgId='+hostOrgId
		}).on({
