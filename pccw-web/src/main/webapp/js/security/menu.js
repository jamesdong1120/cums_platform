var menuManager = function(){
	...
	var init = function(data, item, opreate){
		...
		$.get(baseUrl+'findMenuType.do').done(function(result){
	...
	*   保存数据 id存在则为修改 否则为新增
	...
	var save = function(item){
		...
		if(parentId && parentLevel){
			url =  baseUrl + 'addAndAssignParent.do';
		}
	...
	*获取表单数据
	...
	var changePosition = function(selectItem ,grid, items){
		...
		$.post(contextPath + '/auth/Menu/updateMenuOrder.do', data).done(function(result){
			if(data.actionError){
				grid.message({
	                type: 'error',
	                content: data.actionError
