var roleManager = function(){
	...
	var deleteUser = function(roles, grid){
		...
		dataGrid = grid;
		$.post(baseUrl + 'del.do', data).done(function(data){
	...
	*获取表单数据
	...
	var assignRole = function(userId, userAccount, grid){
		...
		$.get(contextPath + '/pages/auth/select-role.jsp').done(function(data){
			...
			dialog.find('#save').on('click',function(){
				...
				$.post(contextPath + '/auth/User/assignRoles.do', data).done(function(data){
					if(data.result == 'success'){
						dataGrid.message({
							type: 'success',
							content: '保存成功'
	...
	var initSelectRoleGrid = function(userId, userAccount, dialog){
		...
		dialog.find('#selectRoleGrid').grid({
             ...
             url: baseUrl+'queryRolesForAssign.do?userId='+userId+'&userAccount='+userAccount
        });
	};
	/**
	 * 解除角色关联
	...
	var removeRoleForUser = function(userId, roles, grid){
		...
		data.userId = userId;
		dataGrid = grid;
		$.post(baseUrl + 'removeRoleForUser.do', data).done(function(data){
			if(data.result == 'success'){
				dataGrid.message({
					type: 'success',
					content: '删除成功'
	...
	var assignResource = function(grid, roleId){
		$.get(contextPath + '/pages/auth/assign-resource.jsp').done(function(data){
            ...
            dialog.find('#save').on('click',function(){
				...
				$.post(baseUrl + 'assignMenuResources.do', data).done(function(data){
					if(data.result == 'success'){
						grid.message({
							type: 'success',
							content: '保存成功'
	...
	* 加载资源树
	...
	var initResourceTree = function(dialog, roleId){
		$.get(contextPath + '/auth/Menu/findMenuTreeSelectItemByRole.do?time='+new Date().getTime()+'&roleId='+roleId).done(function(result){
