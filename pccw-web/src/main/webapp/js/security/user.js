var userManager = function() {
	...
	var resetPassword = function(item, grid) {
		var dataGrid = grid;
		$.post(baseUrl + 'resetPassword.do?userId=' + item.id).done(function(data) {
	...
	var assignUser = function(roleId, grid) {
		$.get(contextPath + '/pages/auth/select-user.jsp').done(function(data) {
			...
			dialog.find('#save').on('click', function() {
				...
				$.post(contextPath + '/auth/Role/assignUsers.do', data).done(function(data) {
					if (data.result == 'success') {
						grid.message({
							type : 'success',
							content : '保存成功'
	...
	var initSelectUserGrid = function(roleId, dialog) {
		...
		dialog.find('#selectUserGrid').grid({
			...
			url : baseUrl + 'queryUsersForAssign.do?roleId=' + roleId
		});
	};
	/**
	 * 从角色下删除用户
	...
	var removeUserForRole = function(roleId, users, grid) {
		...
		dataGrid = grid;
		data.roleId = roleId;
		$.post(baseUrl + 'removeUserForRole.do', data).done(function(data) {
			if (data.result == 'success') {
				dataGrid.message({
					type : 'success',
					content : '删除成功'
